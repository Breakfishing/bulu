...
window.renderPointsManagementTab = function() {
  ...
  let mainHeader = document.querySelector('#tab-manage > .pm-header') || document.querySelector('#tab-manage > .settings-header-flex');
  if (mainHeader) {
    mainHeader.className = 'pm-header settings-header-flex';
    mainHeader.innerHTML = `
      <span>포인트 관리</span>
      <button type="button" class="pm-add-category-btn" onclick="window.openCategoryAddBottomSheet()" style="background: none; border: none; font-size: 24px; color: var(--text-main); cursor: pointer; padding: 0 4px; display: flex; align-items: center; justify-content: center; font-weight: bold;">+</button>
    `;
  }

  let catSectionHeader = accordionContainer.parentElement.querySelector('.pm-section-title.pm-title-flex-space') || accordionContainer.parentElement.querySelector('.pm-section-title');
  if (catSectionHeader) {
    catSectionHeader.innerHTML = `<span>카테고리</span>`;
  }

  const grouped = combinedPoints.reduce((acc, obj) => { 
    const key = obj.category; 
    if (key === '공중화장실 정보' || key === 'toilet') return acc; 
    if (!acc[key]) acc[key] = []; 
    acc[key].push(obj); 
    return acc; 
  }, {});
  ...
  if (grouped['미분류'] || Object.keys(grouped).length === 0) { if (!grouped['미분류']) grouped['미분류'] = []; }
    ...
    catCard.innerHTML = `
      <div class="pm-cat-header" style="cursor: grab; user-select: none; touch-action: none;">
        <div class="pm-cat-header-left">
          <svg class="pm-chevron" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" stroke-width="3"><polyline points="6 9 12 15 18 9"></polyline></svg>
          <span class="pm-color-dot" style="background:${catColor}"></span>
          <span class="pm-cat-title">${catName} <span class="pm-cat-count">(${catPoints.length})</span></span>
        </div>
