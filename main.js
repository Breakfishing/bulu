...
window.renderPointsManagementTab = function() {
  ...
  let catSectionHeader = accordionContainer.parentElement.querySelector('.pm-header.settings-header-flex') || accordionContainer.parentElement.querySelector('.pm-section-title.pm-title-flex-space');
  if (!catSectionHeader) {
    const titles = accordionContainer.parentElement.querySelectorAll('.pm-section-title, .pm-header');
    for (const title of titles) { if (title.textContent.includes('카테고리')) { catSectionHeader = title; break; } }
  }
  if (catSectionHeader) {
    catSectionHeader.className = 'pm-header settings-header-flex';
    catSectionHeader.innerHTML = `<span>카테고리</span><button type="button" class="pm-add-category-btn" onclick="window.openCategoryAddBottomSheet()">+</button>`;
  }

  const grouped = combinedPoints.reduce((acc, obj) => { const key = obj.category === 'toilet' ? '공중화장실 정보' : obj.category; if (key === '공중화장실 정보') return acc; if (!acc[key]) acc[key] = []; acc[key].push(obj); return acc; }, {});
