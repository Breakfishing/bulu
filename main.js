...
window.renderPointsManagementTab = function() {
  ...
  let mainHeader = document.querySelector('#tab-manage .pm-header');
  if (mainHeader) {
    mainHeader.className = 'pm-header settings-header-flex';
    mainHeader.innerHTML = `
      <span>포인트 관리</span>
      <button type="button" class="pm-add-category-btn" onclick="window.openCategoryAddBottomSheet()" style="background: none; border: none; font-size: 24px; color: var(--text-main); cursor: pointer; padding: 0 4px; display: flex; align-items: center; justify-content: center;">+</button>
    `;
  }
