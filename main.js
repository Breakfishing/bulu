...
window.coastalContoursLayer = null; // SUB-GROUP: 등심선 전역 레이어 보관변수

document.addEventListener("DOMContentLoaded", () => {
  window.initHomeDataSequence();
  window.loadCoastalContoursData(); // 앱 시작 시 등심선 GeoJSON 자동 로드
});

// SUB-GROUP: 등심선 GeoJSON 자동 적재 및 실시간 지도 드로잉 엔진
window.loadCoastalContoursData = async function() {
  try {
    const response = await fetch('coastal_contours.json');
    if (response.ok) {
      const geojsonData = await response.json();
      
      if (window.coastalContoursLayer) {
        map.removeLayer(window.coastalContoursLayer);
      }

      // GeoJSON 선 데이터를 지도에 즉시 주입
      window.coastalContoursLayer = L.geoJSON(geojsonData, {
        style: window.getContourStyle,
        onEachFeature: function (feature, layer) {
          if (feature.properties && feature.properties.depth) {
            // 선 위에 마우스를 대거나 터치하면 수심이 뜨도록 설정
            layer.bindTooltip(`${feature.properties.depth}m`, { permanent: false, sticky: true });
          }
        }
      }).addTo(map);

      console.log("등심선(70m 미만) 선 레이어 지도 시각화 완료");
    } else {
      console.warn("coastal_contours.json 선 데이터를 찾을 수 없습니다.");
    }
  } catch (err) {
    console.error("등심선 로드 중 에러 발생:", err);
  }
};
