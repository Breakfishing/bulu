...
window.renderPointsManagementTab = function() {
  ...
  else { favContainer.innerHTML = ''; favorites.sort((a, b) => (b.favoritedAt || 0) - (a.likedAt || a.favoritedAt || 0)); favorites.forEach(item => { favContainer.appendChild(createPointRowComponent(item, true)); }); bindDragAndDropEvents(favContainer, true); }
  ...
  let catSectionHeader = accordionContainer.parentElement.querySelector('.pm-header.settings-header-flex') || accordionContainer.parentElement.querySelector('.pm-section-title.pm-title-flex-space') || accordionContainer.parentElement.querySelector('.pm-header.pm-title-flex-space');
  if (!catSectionHeader) {
  ...
  const grouped = combinedPoints.reduce((acc, obj) => { 
    const key = obj.category === 'toilet' ? '공중화장실 정보' : obj.category; 
    if (key === '공중화장실 정보') return acc; 
    if (!acc[key]) acc[key] = []; 
    acc[key].push(obj); 
    return acc; 
  }, {});
  ...
  if (savedCatOrder.length === 0) {
    savedCatOrder = [...new Set(cachedFishingPoints.map(p => (p.category || '미분류').trim()))].filter(cat => cat !== '공중화장실 정보' && cat !== 'toilet' && cat !== '미분류');
  }

  savedCatOrder.forEach(cat => { if (!grouped[cat]) grouped[cat] = []; });
  if (grouped['미분류'] || Object.keys(grouped).filter(k => k !== '공중화장실 정보').length === 0) { if (!grouped['미분류']) grouped['미분류'] = []; }
  ...
  Object.keys(grouped).sort((a, b) => {
    ...
    const catPoints = grouped[catName];
    const catColor = catName === '미분류' ? '#868e96' : (catPoints.length > 0 ? (catPoints[0].color || '#007aff') : (savedCatColors[catName] || '#4f46e5'));
    ...
    catCard.innerHTML = `
      <div class="pm-cat-header" style="cursor: grab; user-select: none; touch-action: none;">
        ...
        <div class="pm-item-actions">${catName !== '미분류' ? `
    ...
    accordionContainer.appendChild(catCard); if (catName !== '미분류') bindDragAndDropEvents(bodyContainer, false);
  });
  ...
  accordionContainer.addEventListener('pointerdown', function(e) {
    ...
    const catAttr = targetCard.querySelector('.pm-cat-body')?.getAttribute('data-category'); if (catAttr === '미분류') return;
    ...
    const onCategoryMove = function(evt) {
      ...
      const nextSibling = siblings.find(sibling => { const checkCat = sibling.querySelector('.pm-cat-body')?.getAttribute('data-category'); if (checkCat === '미분류') return false; return evt.clientY < sibling.getBoundingClientRect().top + sibling.getBoundingClientRect().height / 2; });
      const unmovableCard = accordionContainer.querySelector('.pm-cat-card [data-category="미분류"]')?.closest('.pm-cat-card');
