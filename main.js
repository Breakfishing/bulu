...
window.bindDragAndDropEvents = function(container, isFavSection = false) {
 ...
 window.removeEventListener('pointermove', onPointerMove); window.removeEventListener('pointerup', onCategoryUp); window.removeEventListener('pointercancel', onCategoryUp);
 if (isFavSection) saveFavoriteOrderToFirebase(container);
 else saveCategoryOrderWithinTabToFirebase(container);
 };
 function onCategoryUp(evt) {
 item.classList.remove('dragging'); handle.releasePointerCapture(evt.pointerId);
 window.removeEventListener('pointermove', onPointerMove); window.removeEventListener('pointerup', onCategoryUp); window.removeEventListener('pointercancel', onCategoryUp);
 if (isFavSection) saveFavoriteOrderToFirebase(container);
 else saveCategoryOrderWithinTabToFirebase(container);
 }
 window.addEventListener('pointermove', onPointerMove); window.addEventListener('pointerup', onCategoryUp); window.addEventListener('pointercancel', onCategoryUp);
 });
}
...
window.renderPointsManagementTab = function() {
 ...
 // 클릭된 노드 중앙 정렬 대상 수학적 기하 스크롤 벡터 계산 구동
 ...
 // 최초 렌더링 시 마지막에 보았던 탭의 포커스 좌표를 강제 포지셔닝 트랙 보정
